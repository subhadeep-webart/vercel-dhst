...
const WaringModal = ({ submitHandler, isProcessing = false, description = "This action will delete permanently. Please confirm before proceeding.", children }) => {
    ...
    return (
        <>
            <Button onClick={openModal} className="!border-none !p-0 w-fit bg-transparent hover:bg-transparent" size="sm">
                {children}
            </Button>
            <Modal
                ...
                <div className="text-center">
                    ...
                    <div className="flex items-center justify-center w-full gap-3 mt-7">
                        <button
                            onClick={handleSave}
                            type="button"
                            className="flex justify-center w-full px-4 py-3 text-sm font-medium text-white rounded-lg bg-warning-500 shadow-theme-xs hover:bg-warning-600 sm:w-auto cursor-pointer"
                            disabled={isProcessing}
                        >
                            {isProcessing ? "Submitting...." : "Okay, Got It"}
                        </button>
                    </div>
                </div>
            </Modal>
